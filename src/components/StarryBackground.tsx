...
const StarryBackground: React.FC<{ show: boolean, lightDisabled?: boolean }> = ({ show, lightDisabled }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-[-2] transition-opacity duration-300 ${show ? 'opacity-100' : 'opacity-0'} dark:hidden`}>
        <Image 
        ...
        <div 
          ...
          style={{
            background: 'radial-gradient(circle at center, rgba(255,255,255,0) 0%, rgba(255,255,255,0.8) 50%, rgba(255,255,255,1) 100%)',
          }}
        />
      </div>
      ...
      <canvas 
        ref={canvasRef}
        className="fixed inset-0 z-[-1] transition-opacity duration-300 hidden dark:block"
